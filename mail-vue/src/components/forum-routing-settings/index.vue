<template>
  <div class="forum-routing">
    ...
    <el-dialog v-model="visible" :title="t('forumRouting')" width="760px" class="forum-dialog">
      ...
      <div class="grid policy"><div class="field"><label>{{ t('spamScoring') }}</label><el-switch v-model="policy.spamEnabled" :active-value="1" :inactive-value="0"/></div><div class="field"><label>{{ t('cloudflareSpamAlways') }}</label><el-switch v-model="policy.spamCfIsSpam" :active-value="1" :inactive-value="0"/></div><div class="field"><label>{{ t('finalEventOnly') }}</label><el-switch v-model="policy.spamFinalOnly" :active-value="1" :inactive-value="0"/></div><div class="field"><label>{{ t('ignoreEarlyNone') }}</label><el-switch v-model="policy.spamIgnoreNonfinalNone" :active-value="1" :inactive-value="0"/></div><div class="field"><label>{{ t('threshold') }}</label><el-input-number v-model="policy.spamThreshold" :min="1"/></div><div class="field"><label>{{ t('minimumSignals') }}</label><el-input-number v-model="policy.spamMinSignals" :min="1" :max="3"/></div></div>
      <h4>{{ t('authWeights') }}</h4>
      <div class="weight-rows"><div v-for="row in weightRows" :key="row.key" class="weight-row"><strong>{{ row.label }}</strong><div class="weight-grid"><div v-for="status in authStatuses" :key="status" class="field"><label>{{ t(`auth${status[0].toUpperCase()}${status.slice(1)}`) }}</label><el-input-number v-model="weights[row.key][status]" :min="-10" :max="10"/></div></div></div></div>
...
const defaultWeights = () => ({ spf: { pass: 0, none: 2, softfail: 2, fail: 4, temperror: 1, permerror: 3, unknown: 0 }, dkim: { pass: 0, none: 2, softfail: 0, fail: 4, temperror: 1, permerror: 3, unknown: 0 }, dmarc: { pass: 0, none: 2, softfail: 0, fail: 4, temperror: 1, permerror: 3, unknown: 0 } });
const weights = reactive(defaultWeights()); const weightRows = [{ key: 'spf', label: 'SPF' }, { key: 'dkim', label: 'DKIM' }, { key: 'dmarc', label: 'DMARC' }]; const authStatuses = ['pass', 'none', 'softfail', 'fail', 'temperror', 'permerror', 'unknown'];
const copy = (target, source) => Object.assign(target, source || {});
function legacyWeights(settings) {
  const defaults = defaultWeights();
  return {
    spf: { ...defaults.spf, none: Number(settings?.spamSpfNone ?? defaults.spf.none), softfail: Number(settings?.spamSpfSoftfail ?? defaults.spf.softfail), fail: Number(settings?.spamSpfFail ?? defaults.spf.fail) },
    dkim: { ...defaults.dkim, none: Number(settings?.spamDkimNone ?? defaults.dkim.none), fail: Number(settings?.spamDkimFail ?? defaults.dkim.fail) },
    dmarc: { ...defaults.dmarc, none: Number(settings?.spamDmarcNone ?? defaults.dmarc.none), fail: Number(settings?.spamDmarcFail ?? defaults.dmarc.fail) }
  };
}
function loadWeights(raw, settings) { let source; try { source = JSON.parse(raw || ''); } catch (_) { source = null; } source = source?.spf && source?.dkim && source?.dmarc ? source : legacyWeights(settings); const defaults = defaultWeights(); weightRows.forEach(row => authStatuses.forEach(status => { weights[row.key][status] = Number(source[row.key][status] ?? defaults[row.key][status]); })); }
async function reload() { let [config, routeRows, zoneRows, ruleRows, settings] = await Promise.all([forumConfig(), forumRoutes(), forumZones(), spamRules(), settingQuery()]); config = config || {}; copy(forum, { enabled: config.enabled || 0, chatId: config.chat_id || '', defaultNormalThreadId: config.default_normal_thread_id || 0, defaultSpamThreadId: config.default_spam_thread_id || 0 }); routes.value = routeRows || []; zones.value = zoneRows || []; rules.value = ruleRows || []; copy(policy, settings); loadWeights(settings?.spamAuthWeights, settings); }
...
async function savePolicy() { await settingSet({ spamEnabled: policy.spamEnabled, spamThreshold: policy.spamThreshold, spamCfIsSpam: policy.spamCfIsSpam, spamFinalOnly: policy.spamFinalOnly, spamIgnoreNonfinalNone: policy.spamIgnoreNonfinalNone, spamMinSignals: policy.spamMinSignals, spamAuthWeights: JSON.stringify(weights) }); ElMessage.success(t('spamPolicySaved')); }
...
h4{margin:18px 0 8px;font-size:14px}.weight-rows{display:flex;flex-direction:column;gap:12px}.weight-row{padding:10px;border:1px solid var(--el-border-color-lighter);border-radius:6px}.weight-row strong{display:block;margin-bottom:8px}.weight-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:8px}
